...
list_sf_trans <- lapply(no.uses_sf_prj_all, function(x) st_transform(x, "+proj=aea +lat_0=0 +lon_0=132 +lat_1=-18 +lat_2=-36 +x_0=0 +y_0=0 +ellps=GRS80 +towgs84=0,0,0,0,0,0,0 +units=m +no_defs +type=crs
"))
...
pbmclapply(no.uses_sf_prj_crop, rast_uses, mc.cores = length(no.uses_sf_prj_crop), mc.style = "ETA")
...
### Having computer memory issues
### Will run the loops in chunks and combine later
...
# Loop to find intersects
 for(i in 1:nrow(combodf)){
   ...
   first <- no.uses_sf_prj_crop[[combo$x]] # Indicate where to find first industry
   second <- no.uses_sf_prj_crop[[combo$y]] # Indicate where to find second industry
...
# Save
write_csv(matrix_table, here('output_data', 'ocean_uses', 'matrix_table.csv'))
